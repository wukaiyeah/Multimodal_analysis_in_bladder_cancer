...
datatable[,c(5:ncol(datatable))] <- apply(datatable[,c(5:ncol(datatable))],2,SCALE)

datatable[,5:ncol(datatable)] = scale(datatable[,5:ncol(datatable)])

##---feature list
# filter features, load res of t.test
feature_test_res = read.table('/media/wukai/AI_Team_03/Bladder_project/FeatureAssociations/grade_pval_res_redund.txt', header = TRUE, sep = '\t')

...
lamb=0     #default value sparsity regularization parameter
...
if op==0:
	...
	input_direc="/share/Data01/wupeng2/pathology/out/filter/114333_1.svs/"
	output_direc="/share/Data01/wupeng2/pathology/out/filter/test_out"
	...
	level=0#这里指的是svs文件中有多少层图片

	# input_direc="../experiment_CN/"
	input_direc="/share/Data01/wupeng2/pathology/TCGA/filter/"
	output_direc="/share/Data01/wupeng2/pathology/normal/"
	if not os.path.exists(output_direc):
		os.makedirs(output_direc)
	file_type="*.*"
	...
	all_dir = os.listdir(input_direc)
	all_files = []
	for root, dirs, files in os.walk(input_direc):
		if files != []:
			all_files = all_files + [root + "/" + k for k in files]

	filenames=[target_filename]+sorted(all_files[482:])

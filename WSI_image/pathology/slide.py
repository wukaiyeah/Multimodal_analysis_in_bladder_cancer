...
BASE_DIR = "./beijing"
BASE_DIR2 = "/share/service04/wukai/WSI_image/WSI_beijing/"
# BASE_DIR = os.path.join(os.sep, "Volumes", "BigData", "TUPAC")

# SRC_TRAIN_DIR = os.path.join(BASE_DIR2, "/")
SRC_TRAIN_DIR = BASE_DIR2
SRC_TRAIN_EXT = "svs"
DEST_TRAIN_SUFFIX = ""  # Example: "train-"
DEST_TRAIN_EXT = "jpg"
SCALE_FACTOR = 4
DEST_TRAIN_DIR = os.path.join(BASE_DIR, "tissue_slide_images/filter/" + DEST_TRAIN_EXT)
THUMBNAIL_SIZE = 1440
THUMBNAIL_EXT = "jpg"
...
FILTER_DIR = os.path.join(BASE_DIR, "tissue_slide_images/filter/jpg/")
...
def get_training_slide_path(slide_number):
  ...
  slide_filepath = os.path.join(SRC_TRAIN_DIR, slide_number)
...
def get_tile_image_path(tile):
  ...
  tile_path = os.path.join(TILE_DIR, padded_sl_num,
                           padded_sl_num + "-" + TILE_SUFFIX + "-r%d-c%d-x%d-y%d-w%d-h%d" % (
...
def get_training_thumbnail_path(slide_number, large_w=None, large_h=None, small_w=None, small_h=None):
  ...
  padded_sl_num = str(slide_number).zfill(3)
  if large_w is None and large_h is None and small_w is None and small_h is None:
    wilcard_path = os.path.join(DEST_TRAIN_THUMBNAIL_DIR, added_sl_num + "*." + THUMBNAIL_EXT)
    img_path = glob.glob(wilcard_path)[0]
  else:
    img_path = os.path.join(DEST_TRAIN_THUMBNAIL_DIR, padded_sl_num + "-" + str(
      SCALE_FACTOR) + "x-" + DEST_TRAIN_SUFFIX + str(
...
def get_filter_image_filename(slide_number, filter_number, filter_name_info, thumbnail=False):
  ...
  img_filename = padded_sl_num + "-" + padded_fi_num + "-" + FILTER_SUFFIX + filter_name_info + "." + ext
  return img_filename
...
def get_filter_thumbnail_result(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  img_path = os.path.join(FILTER_THUMBNAIL_DIR, padded_sl_num + "-" + str(
    SCALE_FACTOR) + "x-" + FILTER_SUFFIX + str(large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(
...
def get_training_slides():
  ...
  """
  training_slides = glob.glob1(SRC_TRAIN_DIR, "*." + SRC_TRAIN_EXT)
  num_training_slides = len(training_slides)
  return num_training_slides, training_slides
...
def training_slide_range_to_images(start_ind, end_ind):
  ...
  for slide_num in range(start_ind, end_ind + 1):
    training_slide_to_image(training_slides[slide_num])
...
def singleprocess_training_slides_to_images():
  ...
  num_train_images, _ = get_training_slides()
...
def slide_stats():
  ...
  num_train_images, train_images = get_training_slides()
  slide_stats = []
  for slide_num in train_images:
    slide_filepath = get_training_slide_path(slide_num)
    print("Opening Slide #%s" % (slide_num))
    slide = open_slide(slide_filepath)
